...
| 유휴 CPU | ✅ 15초간 0ms (release) |
| 인스톨러 | ✅ NSIS 2.83MB, release exe 7.0MB, Vite 없이 독립 실행 |
| 메모리 (release, private bytes) | ⚠️ **177.3MB — PRD 목표 150MB 미달성** |

### 메모리 목표 미달성

앱 프로세스는 private 7.6MB 뿐이고, 나머지 169.7MB 는 WebView2 6개 프로세스다.
PRD 의 150MB 는 WebView2 의 프로세스 분리 구조를 모르고 잡은 값으로 보이며,
앱 코드로 줄일 여지가 거의 없다. 목표 상향 또는 네이티브 UI 교체 중 선택이 필요하다
(상세: [tasks.md](tasks.md) M6).

WorkingSet(384.1MB)은 WebView2 프로세스 간 공유 페이지를 중복 계산하므로
비교 기준으로 쓰지 않는다.
