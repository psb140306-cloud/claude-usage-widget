...
- [ ] **위치 저장·복원** — `widgetPosition` 필드는 있으나 아무도 쓰지 않는다. 매 실행 우측 하단 고정
  - 기본 위치는 구현됨: 작업 영역 우측 하단 여백 16px (`place_bottom_right`)
...
- [ ] **다크/라이트/시스템 테마 적용** — `theme` 필드는 있으나 **적용 코드도 UI도 없다**
  - 현재는 OS `prefers-color-scheme` 를 그대로 따르고, 색상은 설정에서 직접 바꾸는 방식
...
| # | 항목 | 위치 |
|---|---|---|
| 1 | 다크/라이트/시스템 **테마 적용** | 4.2 — `theme` 필드만 있고 적용 코드·UI 없음 |
| 2 | 위젯 **위치 저장·복원** | 3.1 — `widgetPosition` 필드만 있고 쓰는 곳 없음 |
| 3 | **일별/주별 집계 뷰** (요일 패턴) | 5.2 |
